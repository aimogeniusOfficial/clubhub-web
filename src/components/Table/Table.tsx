...
import { Table, ScrollArea, ScrollAreaProps, Avatar, Stack, Skeleton } from '@mantine/core';
    ...
    <>
      <ScrollArea {...props}>
          ...
          <thead>
            {headerGroups.map((headerGroup: any) => (
              <tr {...headerGroup.getHeaderGroupProps()}>
                <TableHeader key={headerGroup.id} columns={headerGroup.headers} />
              </tr>
            ))}
          </thead>
          <tbody>
        ...
        {!isLoading && data.length === 0 && (
          <EmptyState mt='xl' title='Table is empty' description='There is no data to display.' />
        )}
      </ScrollArea>
    </>
  );
