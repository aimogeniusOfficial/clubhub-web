...
const OtherPlantCultivarTable = (): ReactElement => {
  const theme = useMantineTheme();
  const { data: tableData } = useGrowerCultivars('');
  ...
  const columns = [
    ...
    {
      Header: 'Plant Type',
      accessor: 'plantTypeName',
      Cell: ({ row }: { row: any }) => <>{`${row.original.plantType}`}</>,
    },
    {
...
export default OtherPlantCultivarTable;
